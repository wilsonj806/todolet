...
const findUserWithUsername: RequestHandler = async (
  req,
  res,
  next
): Promise<any> => {
  try {
    ...
    if (user) {
      res
        ...
        .json(
          responsifyError(
            `Error: User with username: ${username} exists already`
          )
        );
    } else {
      next();
    ...
    res
      .status(500)
      .json(responsifyError("Error: Internal server error, sorry :("));
    console.error("Error alert, check below for additional logging \n", error);
  }
};

const postRegisterFailure: ErrorRequestHandler = (
  err,
  req,
  res,
  next
...
export { findUserWithUsername, postRegisterFailure };
